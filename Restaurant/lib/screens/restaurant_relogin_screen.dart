...
class _RestaurantReloginScreenState extends State<RestaurantReloginScreen> {
  ...
  void initState() {
    super.initState();
    _mobileController = TextEditingController(text: widget.initialPhone ?? '9876543210');
  }
  ...
  Future<void> _handlePinLogin(BuildContext sheetContext, String pin) async {
    ...
    const restaurantId = "mock_restaurant_pin";
    const token = "mock_token_pin";
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('PIN Verified! Welcome back to ECDKART Partner.'),
        backgroundColor: primaryGreen,
      ),
    );
    Navigator.pushReplacement(
