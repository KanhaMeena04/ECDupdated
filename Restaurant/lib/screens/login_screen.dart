...
class _LoginScreenState extends State<LoginScreen> {
  ...
  bool _isPinLoginMode = true;
  bool _isLoginPinObscured = true;
  ...
  final List<TextEditingController> _regOtpControllers =
      List.generate(6, (_) => TextEditingController());
  final List<FocusNode> _regOtpFocusNodes =
      List.generate(6, (_) => FocusNode());

  // 6-digit Login OTP Controllers & Focus Nodes
  final List<TextEditingController> _loginOtpControllers =
      List.generate(6, (_) => TextEditingController());
  final List<FocusNode> _loginOtpFocusNodes =
      List.generate(6, (_) => FocusNode());
  ...
  Future<bool> _isExistingRestaurant(String phone) async {
    ...
    final registeredList =
        prefs.getStringList('registered_restaurant_phones') ?? [];
    return registeredList.contains(cleanPhone);
  }

  // Restaurant Details Controllers (Step 1 - clean blank fields)
  ...
  static const String _googleMapsApiKey =
      'AIzaSyCN7XqyxOj5lgr2uaMNrTOg6PzHTOGa0xU';
  bool _isFetchingLocation = false;
  String _selectedPinLabel = '';

  // Location Controllers (Step 2 - clean blank fields)
  ...
  // Documents Controllers (Step 3 - clean blank fields)
  final _foodLicenseController = TextEditingController();
  final _gstNumberController = TextEditingController();
  String? _uploadedFoodLicenseDoc;
  String? _uploadedFoodLicenseDocData;
  String? _uploadedGstDoc;
  String? _uploadedGstDocData;

  // Bank Details Controllers (Step 4 - clean blank fields)
  ...
  // Add Menu Controllers & State (Step 6 - clean blank fields)
  final _menuItemNameController = TextEditingController();
  final _menuBasePriceController = TextEditingController();
  final _menuDescriptionController = TextEditingController();
  String _selectedMenuCategory = 'Main Course';
  String _selectedFoodType = 'Veg';
  String? _uploadedMenuItemImage;
  String? _uploadedMenuItemImageData;
  final List<String> _flavourVariants = [];
  final List<String> _addOnsList = [];
  final List<Map<String, dynamic>> _addedMenuItems = [];

  // Step 7: 4-Digit Login PIN Controllers
  ...
  Future<void> _pickImageWithSource({
    required ImageSource source,
    required Function(String name, String base64Data) onSelected,
  }) async {
    try {
      ...
      if (file != null) {
        ...
        onSelected(file.name, base64Data);
      }
    } catch (e) {
      if (mounted) {
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              title: Text('Live Capture (Camera)',
                  style: GoogleFonts.poppins(
                      fontWeight: FontWeight.w700, fontSize: 14)),
              subtitle: Text('Take a real live photo with your camera',
                  style: GoogleFonts.poppins(
                      fontSize: 12, color: Colors.grey[600])),
              onTap: () {
                Navigator.pop(ctx);
                _pickImageWithSource(
                    source: ImageSource.camera, onSelected: onSelected);
              },
            ),
            const Divider(),
            ListTile(
              ...
              title: Text('Upload from Device Gallery / Storage',
                  style: GoogleFonts.poppins(
                      fontWeight: FontWeight.w700, fontSize: 14)),
              subtitle: Text('Pick image file directly from your device',
                  style: GoogleFonts.poppins(
                      fontSize: 12, color: Colors.grey[600])),
              onTap: () {
                Navigator.pop(ctx);
                _pickImageWithSource(
                    source: ImageSource.gallery, onSelected: onSelected);
              },
            ),
            const SizedBox(height: 12),
          ],
  ...
  Future<void> _handleRegister() async {
    ...
    if (mounted) {
      setState(() {
        ...
        _regOtpControllers[0].text = '5';
      });
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('6-Digit Verification Code sent to your Email/SMS!'),
  ...
  Future<void> _handleVerifyRegisterOtp() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text(
                'OTP Verified Successfully! Please fill in your Restaurant Details.'),
            backgroundColor: AppTheme.primaryGreen),
      );
    }
  }
  ...
  Future<void> _handleSaveRestaurantDetails() async {
    ...
    if (tradeName.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Please enter your Restaurant Name'),
            backgroundColor: Colors.red),
      );
      return;
    }
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text(
                'Step 1 Saved! Step 2: Please enter your Location details.'),
            backgroundColor: AppTheme.primaryGreen),
      );
    }
  }
  ...
  // ----------------------------------------------------
  // GOOGLE MAPS & LOCATION REVERSE GEOCODING LOGIC
  // ----------------------------------------------------
  ...
  Future<List<Map<String, dynamic>>> _searchGoogleAddress(String query) async {
        ...
        if (data['status'] == 'OK') {
          ...
          return results.map<Map<String, dynamic>>((r) {
            ...
            return {
              ...
              'lat': (geom['lat'] as num?)?.toDouble() ?? 28.2478,
              'lng': (geom['lng'] as num?)?.toDouble() ?? 77.0624,
              'area': area,
              'city': city,
            };
          }).toList();
  ...
  void _showMapLocationPickerModal() {
    final searchCtrl = TextEditingController();
    List<Map<String, dynamic>> searchResults = [];
    bool isSearching = false;
    double tempLat =
        double.tryParse(_latitudeController.text.trim()) ?? 28.2478;
    double tempLng =
        double.tryParse(_longitudeController.text.trim()) ?? 77.0624;
    String tempAddress = _addressController.text.trim().isNotEmpty
        ? _addressController.text.trim()
        : 'Subhash Chowk, Sohna, Haryana';
    String tempArea = _areaController.text.trim().isNotEmpty
        ? _areaController.text.trim()
        : 'Subhash Chowk';
    String tempCity = _cityController.text.trim().isNotEmpty
        ? _cityController.text.trim()
        : 'Sohna';

    final popularLandmarks = [
      {
        'title': 'Subhash Chowk, Sohna',
        'area': 'Subhash Chowk',
        'city': 'Sohna',
        'lat': 28.2478,
        'lng': 77.0624
      ...
      {
        'title': 'Sector 48, Sohna Road, Gurugram',
        'area': 'Sector 48',
        'city': 'Gurugram',
        'lat': 28.4180,
        'lng': 77.0425
            ...
            return Container(
              ...
              child: Column(
                children: [
                          ...
                          child: Container(
                            ...
                            child: TextField(
                              ...
                              decoration: InputDecoration(
                                hintText: 'Search address, colony, market...',
                              ...
                              onSubmitted: (val) async {
                                ...
                                final results =
                                    await _searchGoogleAddress(val.trim());
                                setModalState(() {
                            ...
                            setModalState(() {
                              ...
                              tempLng =
                                  double.tryParse(_longitudeController.text) ??
                                      77.0624;
                              tempAddress = _addressController.text;
                              tempArea = _areaController.text;
                              tempCity = _cityController.text;
                              isSearching = false;
                            });
                          },
                          borderRadius: BorderRadius.circular(12),
                          ...
                          return ListTile(
                            ...
                            onTap: () {
                              setModalState(() {
                                tempLat = item['lat'];
                                tempLng = item['lng'];
                                tempAddress = item['formatted_address'];
                                tempArea = item['area'].isNotEmpty
                                    ? item['area']
                                    : tempArea;
                                tempCity = item['city'].isNotEmpty
                                    ? item['city']
                                    : tempCity;
                                searchResults = [];
                              });
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        SingleChildScrollView(
                          ...
                          child: Row(
                            children: popularLandmarks.map((lm) {
                              final isSelected =
                                  (tempLat - (lm['lat'] as double)).abs() <
                                          0.001 &&
                                      (tempLng - (lm['lng'] as double)).abs() <
                                          0.001;
                              return Padding(
                                padding: const EdgeInsets.only(right: 8),
                                child: ChoiceChip(
                                  label: Text(lm['title'] as String),
                                  selected: isSelected,
                                  ...
                                  onSelected: (_) {
                                    setModalState(() {
                                      tempLat = lm['lat'] as double;
                                      tempLng = lm['lng'] as double;
                                      tempAddress = lm['title'] as String;
                                      tempArea = lm['area'] as String;
                                      tempCity = lm['city'] as String;
                                    });
                                  },
                                ),
                              );
                  ...
                  // Visual Map Canvas Simulation with Live Target Pin
                  Expanded(
                    child: Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const Icon(Icons.arrow_drop_down_rounded,
                                      color: AppTheme.darkBlack, size: 28),
                                ],
                              ),
                            ),
                      ...
                      child: ElevatedButton(
                        onPressed: () {
                          setState(() {
                            ...
                            _addressController.text = tempAddress;
                            _areaController.text = tempArea;
                            _cityController.text = tempCity;
                            _selectedPinLabel = tempAddress;
                          });
                          Navigator.pop(ctx);
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text('📍 Location Pinned: $tempAddress'),
                              backgroundColor: AppTheme.primaryGreen,
                            ),
                          );
                        },
  ...
  Future<void> _handleSaveRestaurantLocation() async {
    ...
    if (address.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Please enter your Restaurant Address'),
            backgroundColor: Colors.red),
      );
      return;
    }
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text(
                'Location Saved! Step 3: Please upload your Restaurant Documents.'),
            backgroundColor: AppTheme.primaryGreen),
      );
    }
  }
  ...
  Future<void> _handleSaveRestaurantDocuments() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content:
                Text('Step 3 Saved! Step 4: Please enter your Bank Details.'),
            backgroundColor: AppTheme.primaryGreen),
      );
    }
  }
  ...
  Future<void> _handleSaveRestaurantBankDetails() async {
    ...
    if (holderName.isEmpty ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Please fill all Bank Details fields'),
            backgroundColor: Colors.red),
      );
      return;
    }
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text(
                'Bank Details Saved! Step 5: Please configure Operational Details.'),
            backgroundColor: AppTheme.primaryGreen),
      );
    }
  }
  ...
  Future<void> _handleSaveRestaurantOperationalDetails() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text(
                'Operational Details Saved! Step 6: Please add Menu Items.'),
            backgroundColor: AppTheme.primaryGreen),
      );
    }
  }
  ...
  void _addItemToMenuList() {
    ...
    if (itemName.isEmpty || price.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content:
              Text('Please enter Item Name and Base Price to add this item.'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    setState(() {
      _addedMenuItems.add({
        ...
        'image': _uploadedMenuItemImageData ?? _uploadedMenuItemImage,
      ...
      _uploadedMenuItemImageData = null;
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
            'Item $itemName added to Menu! (${_addedMenuItems.length} total) Add more items or tap Next.'),
        backgroundColor: AppTheme.primaryGreen,
      ),
    );
  }
  ...
  Future<void> _handleSaveRestaurantAddMenu() async {
    ...
    if (_addedMenuItems.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Please add at least 1 menu item to continue'),
            backgroundColor: Colors.red),
      );
      return;
    }
    ...
    setState(() {
      _currentMode = AuthMode.restaurantSetPin;
    });
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
          content: Text('Menu Saved! Step 7: Set your 4-Digit Login PIN.'),
          backgroundColor: AppTheme.primaryGreen),
    );
  }

  Future<void> _handleSaveRestaurantPin() async {
    final createPin = _createPinController.text.trim();
    final confirmPin = _confirmPinController.text.trim();

    if (createPin.length != 4 || !RegExp(r'^\d{4}$').hasMatch(createPin)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
    ...
    if (createPin != confirmPin) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('PINs do not match! Please check and confirm'),
            backgroundColor: Colors.red),
      );
      return;
    }

    _pinController.text = createPin;

    setState(() {
      _currentMode = AuthMode.restaurantReview;
    });
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
          content: Text(
              'PIN Set Successfully! Final Step: Review Details & Submit.'),
          backgroundColor: AppTheme.primaryGreen),
    );
  }

  Future<void> _handleFinalRestaurantSubmit() async {
    setState(() => _isLoading = true);
    final phone = _mobileController.text.trim().isNotEmpty
        ? _mobileController.text.trim().replaceAll(RegExp(r'\D'), '')
        : (_regMobileController.text.trim().isNotEmpty
            ? _regMobileController.text.trim().replaceAll(RegExp(r'\D'), '')
            : '8305370330');
    final nameStr = _tradeNameController.text.trim().isNotEmpty
        ? _tradeNameController.text.trim()
        : 'New Restaurant';
    final pinStr = _createPinController.text.trim().isNotEmpty
        ? _createPinController.text.trim()
    ...
    final ownerName = fullName.isNotEmpty
        ...
        : (_accountHolderController.text.trim().isNotEmpty
            ...
            : nameStr);
    final ownerEmail = _regEmailController.text.trim().isNotEmpty
        ? _regEmailController.text.trim()
        : (phone.isNotEmpty
            ? 'vendor_$phone@ecdkart.com'
            : 'vendor_${DateTime.now().millisecondsSinceEpoch}@ecdkart.com');
    ...
    try {
      final payload = {
        ...
        'pin': pinStr,
        'name': {'en': nameStr},
        'description': {'en': _aboutRestaurantController.text.trim()},
        ...
        'documents': {
          'license': {
            'number': _foodLicenseController.text.trim(),
            'file':
                _uploadedFoodLicenseDocData ?? _uploadedFoodLicenseDoc ?? '',
          },
          'gst': {
            'number': _gstNumberController.text.trim(),
            'file': _uploadedGstDocData ?? _uploadedGstDoc ?? '',
          },
        },
        'operationalHours': _daysSchedule,
        'menuItems': _addedMenuItems,
      };

      final response = await http
          .post(
            Uri.parse(ApiConstants.restaurantApply),
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        final restObj = resData['restaurant'];
        final restaurantId =
            resData['restaurantId'] ?? restObj?['_id'] ?? restObj?['id'] ?? '';
        final token = resData['token'] ?? resData['authToken'] ?? '';

        if (restaurantId.isNotEmpty && token.isNotEmpty) {
  ...
  Future<void> _handleSendOtp() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final resData = jsonDecode(response.body);
        if (resData['testOtp'] != null) {
          final testOtp = resData['testOtp'].toString();
          for (int i = 0;
              i < testOtp.length && i < _loginOtpControllers.length;
              i++) {
            _loginOtpControllers[i].text = testOtp[i];
          }
        }
        if (mounted) {
          setState(() {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                  resData['message'] ?? 'OTP sent successfully to +91 $phone!'),
              backgroundColor: AppTheme.primaryGreen,
            ),
          );
        }
    ...
    try {
      final response = await http
          .post(
            ...
            headers: {'Content-Type': 'application/json'},
            body: jsonEncode({'mobile': phone, 'otp': otpCode}),
          )
          .timeout(const Duration(seconds: 12));
      ...
      if (response.statusCode == 200) {
        ...
        final bool isApproved =
            data['isApproved'] == true || data['restaurantApproved'] == true;
        final bool hasRestaurant =
            data['restaurant'] != null || restaurantId.isNotEmpty;

        String restName = '';
        if (data['name'] != null && data['name'].toString().isNotEmpty) {
          final n = data['name'];
          restName = (n is Map ? (n['en'] ?? '') : n).toString();
        } else if (data['restaurant'] != null &&
            data['restaurant']['name'] != null) {
          final n = data['restaurant']['name'];
          restName = (n is Map ? (n['en'] ?? '') : n).toString();
        } else if (data['user'] != null && data['user']['name'] != null) {
          restName = data['user']['name'].toString();
        }
        ...
        if (token.isNotEmpty && restaurantId.isNotEmpty) {
          ...
          await prefs.setString('token', token);
          await prefs.setString('userPhone', phone);
          if (restName.isNotEmpty && restName != 'null') {
            await prefs.setString('restaurantName', restName);
          }
        }
        ...
        if (mounted) {
          ...
          if (!hasRestaurant) {
            // New Restaurant Partner -> Go to onboarding
            _regMobileController.text = phone;
            setState(() => _currentMode = AuthMode.restaurantDetails);
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                  content:
                      Text('Mobile verified! Please fill restaurant details.'),
                  backgroundColor: AppTheme.primaryGreen),
            );
          } else if (!isApproved) {
            // Pending Approval -> Go to Waiting Screen
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(
                builder: (context) => WaitingForApprovalScreen(
                  restaurantId: restaurantId,
                  restaurantName: restName.isNotEmpty
                      ? restName
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                  content: Text(
                      'Welcome back, ${restName.isNotEmpty ? restName : "Partner"}!'),
                  backgroundColor: AppTheme.primaryGreen),
            );
            Navigator.pushReplacement(
              context,
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text(err['message'] ?? 'Invalid OTP entered.'),
                backgroundColor: Colors.red),
          );
        }
      }
    ...
    try {
      final response = await http
          .post(
            ...
            headers: {'Content-Type': 'application/json'},
            body: jsonEncode({'mobile': phone, 'pin': pin}),
          )
          .timeout(const Duration(seconds: 12));
      ...
      if (response.statusCode == 200) {
          ...
          if (!isApproved) {
            Navigator.pushReplacement(
              ...
              MaterialPageRoute(
                builder: (context) => WaitingForApprovalScreen(
                  restaurantId: restaurantId,
                  restaurantName: data['restaurant']?['name']?['en'] ??
                      data['restaurant']?['name'],
                  mobile: phone,
                ),
              ),
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                  content: Text('Logged in successfully with PIN!'),
                  backgroundColor: AppTheme.primaryGreen),
            );
            Navigator.pushReplacement(
              context,
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content:
                    Text(err['message'] ?? 'Incorrect Mobile number or PIN'),
                backgroundColor: Colors.red),
          );
        }
      }
  ...
  Widget _buildImagePreview(String src,
  ...
  void _showImageSourceDialog() {
    ...
    _showMediaPickerBottomSheet(
      title: 'Add Restaurant Photo',
      onSelected: (name, base64Data) {
        setState(() {
          _restaurantImages.add(base64Data);
        });
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text('Restaurant photo added! (${_restaurantImages.length}/6)'),
            backgroundColor: AppTheme.primaryGreen,
          ),
        );
      },
                          ...
                          child: Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  uploadedAddOnDoc != null
                                      ? 'Image Uploaded: $uploadedAddOnDoc'
                                      : 'Upload Image',
                                  style: GoogleFonts.poppins(
                                    fontSize: 12,
                                    fontWeight: FontWeight.w600,
                            ...
                            onPressed: () {
                              ...
                              if (name.isNotEmpty) {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content:
                                        Text('Please enter an add-on name'),
                                    backgroundColor: Colors.red,
                                  ),
                                );
                              }
  ...
  String _getHeaderImageAsset() {
    switch (_currentMode) {
      ...
      case AuthMode.restaurantSetPin:
        return 'assets/images/restaurant_login_header.jpg';
      case AuthMode.restaurantReview:
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            // Top Header Section
            if (_currentMode == AuthMode.restaurantReview) ...[
              // Restaurant Review Header (Step 8: Review & Submit application)
              Stack(
                children: [
                  Container(
                    height: 220,
                          ...
                          CircleAvatar(
                            ...
                            child: IconButton(
                              ...
                              icon: const Icon(Icons.arrow_back_rounded,
                                  color: AppTheme.darkBlack, size: 20),
                              onPressed: () {
                                setState(() {
                                  _currentMode = AuthMode.restaurantSetPin;
                                });
            ...
            ] else if (_currentMode == AuthMode.restaurantSetPin) ...[
              // Restaurant Set PIN Header (Step 7: 4-digit Login PIN)
              Stack(
                children: [
                  Container(
                    height: 220,
            ...
            ] else if (_currentMode == AuthMode.restaurantAddMenu) ...[
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          CircleAvatar(
                            ...
                            child: IconButton(
                              ...
                              onPressed: () {
                                setState(() {
                                  _currentMode =
                                      AuthMode.restaurantOperationalDetails;
                                });
                              },
                            ),
                          ),
                          ...
                          // 8 Step Progress Indicator Bar (Steps 1..6 Green)
                          Row(
                            children: List.generate(8, (index) {
                              return Expanded(
                                child: Container(
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // 8 Step Progress Indicator Bar (Steps 1..5 Green)
                          Row(
                            children: List.generate(8, (index) {
                              return Expanded(
                                child: Container(
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // 8 Step Progress Indicator Bar (Steps 1..4 Green)
                          Row(
                            children: List.generate(8, (index) {
                              return Expanded(
                                child: Container(
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // 8 Step Progress Indicator Bar (Steps 1..3 Green)
                          Row(
                            children: List.generate(8, (index) {
                              return Expanded(
                                child: Container(
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // 8 Step Progress Indicator Bar (Steps 1 & 2 Green)
                          Row(
                            children: List.generate(8, (index) {
                              return Expanded(
                                child: Container(
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // 8 Step Progress Indicator Bar (Step 1 Green)
                          Row(
                            children: List.generate(8, (index) {
                              return Expanded(
                                child: Container(
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  // MODE 14: RESTAURANT ONBOARDING STEP 8 - REVIEW DETAILS & SUBMIT (NEW)
                  if (_currentMode == AuthMode.restaurantReview) ...[
                    ...
                    const SizedBox(height: 4),
                    Text(
                      'Please verify all information before submitting for Admin approval.',
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.circular(14),
                        border: Border.all(
                            color: Colors.grey[250] ?? Colors.grey[300]!),
                        boxShadow: [
                          BoxShadow(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                              'Name: ${_tradeNameController.text.trim().isNotEmpty ? _tradeNameController.text.trim() : "Not specified"}',
                              style: GoogleFonts.poppins(
                                  fontSize: 13, fontWeight: FontWeight.w600)),
                          const SizedBox(height: 3),
                          Text('Type: $_selectedRestaurantType',
                              style: GoogleFonts.poppins(
                                  fontSize: 12, color: Colors.grey[700])),
                          const SizedBox(height: 3),
                          Text('Photos: ${_restaurantImages.length} uploaded',
                              style: GoogleFonts.poppins(
                                  fontSize: 12, color: Colors.grey[700])),
                          if (_aboutRestaurantController.text
                              .trim()
                              .isNotEmpty) ...[
                            const SizedBox(height: 3),
                            Text(
                                'About: ${_aboutRestaurantController.text.trim()}',
                                style: GoogleFonts.poppins(
                                    fontSize: 12, color: Colors.grey[600]),
                                maxLines: 2,
                                overflow: TextOverflow.ellipsis),
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const Divider(height: 12),
                          Text(
                              'Address: ${_addressController.text.trim().isNotEmpty ? _addressController.text.trim() : "Not specified"}',
                              style: GoogleFonts.poppins(
                                  fontSize: 13, fontWeight: FontWeight.w600)),
                          const SizedBox(height: 3),
                          Text(
                              'Area / City: ${_areaController.text.trim()}, ${_cityController.text.trim()}',
                              style: GoogleFonts.poppins(
                                  fontSize: 12, color: Colors.grey[700])),
                          if (_latitudeController.text.isNotEmpty &&
                            ...
                            const SizedBox(height: 3),
                            Text(
                                'Coords: ${_latitudeController.text.trim()}, ${_longitudeController.text.trim()}',
                                style: GoogleFonts.poppins(
                                    fontSize: 12, color: Colors.grey[600])),
                          ],
                        ],
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                            ...
                            children: [
                              ...
                              TextButton(
                                onPressed: () => setState(() => _currentMode =
                                    AuthMode.restaurantBankDetails),
                                style: TextButton.styleFrom(
                                    padding: EdgeInsets.zero,
                                    minimumSize: const Size(40, 24)),
                                child: Text('Edit',
                          ...
                          const Divider(height: 12),
                          Text(
                              'Bank: ${_bankNameController.text.trim()} (${_accountHolderController.text.trim()})',
                              style: GoogleFonts.poppins(
                                  fontSize: 13, fontWeight: FontWeight.w600)),
                          const SizedBox(height: 3),
                          Builder(
                            builder: (context) {
                              ...
                              final maskedAcc = acc.length > 4
                                  ? '****${acc.substring(acc.length - 4)}'
                                  : acc;
                              return Text(
                                  'A/C Number: $maskedAcc | IFSC: ${_ifscCodeController.text.trim()}',
                                  style: GoogleFonts.poppins(
                                      fontSize: 12, color: Colors.grey[700]));
                            },
                          ),
                          ...
                          Builder(
                            builder: (context) {
                              final lic = _foodLicenseController.text.trim();
                              final licStr = lic.isNotEmpty ? lic : 'None';
                              final docStr = _uploadedFoodLicenseDoc != null
                                  ? ' (Doc Attached ✓)'
                                  : '';
                              return Text('Food License: $licStr$docStr',
                                  style: GoogleFonts.poppins(
                                      fontSize: 12, color: Colors.grey[700]));
                            },
                          ),
                          ...
                          Builder(
                            builder: (context) {
                              ...
                              final gstStr = gst.isNotEmpty ? gst : 'None';
                              final docStr = _uploadedGstDoc != null
                                  ? ' (Doc Attached ✓)'
                                  : '';
                              return Text('GST Reg: $gstStr$docStr',
                                  style: GoogleFonts.poppins(
                                      fontSize: 12, color: Colors.grey[700]));
                            },
                          ),
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const Divider(height: 12),
                          Text(
                              'Dishes Added: ${_addedMenuItems.length} Menu Items',
                              style: GoogleFonts.poppins(
                                  fontSize: 13, fontWeight: FontWeight.w600)),
                          const SizedBox(height: 3),
                          Text(
                              '4-Digit PIN: • • • • (Securely configured for instant login)',
                              style: GoogleFonts.poppins(
                                  fontSize: 12, color: Colors.grey[700])),
                        ],
                      ),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed:
                            _isLoading ? null : _handleFinalRestaurantSubmit,
                        style: ElevatedButton.styleFrom(
                          backgroundColor: AppTheme.primaryGreen,
                          foregroundColor: Colors.white,
                          elevation: 0,
                  ...
                  // MODE 13: RESTAURANT ONBOARDING STEP 7 - SET 4-DIGIT LOGIN PIN (NEW)
                  else if (_currentMode == AuthMode.restaurantSetPin) ...[
                    Text(
                      'Create 4-Digit Login PIN',
                      style: GoogleFonts.poppins(
                        fontSize: 22,
                        fontWeight: FontWeight.w800,
                        color: AppTheme.darkBlack,
                    ...
                    const SizedBox(height: 6),
                    Text(
                      'Set a secure 4-digit PIN to quickly log into your partner dashboard anytime.',
                      textAlign: TextAlign.center,
                      style: GoogleFonts.poppins(
                          fontSize: 12, color: Colors.grey[600]),
                    ),
                    ...
                    // Enter 4-Digit PIN
                    _buildFormLabel('Enter 4-Digit PIN'),
                    const SizedBox(height: 6),
                    _buildCustomTextField(
                    ...
                    // Confirm 4-Digit PIN
                    _buildFormLabel('Confirm 4-Digit PIN'),
                    const SizedBox(height: 6),
                    _buildCustomTextField(
                      controller: _confirmPinController,
                      hintText: 'Re-enter 4-digit PIN to confirm',
                      keyboardType: TextInputType.number,
                      maxLength: 4,
                    ...
                    // Next: Review Application Button
                    SizedBox(
                      width: double.infinity,
                      height: 52,
                      child: ElevatedButton(
                        onPressed: _handleSaveRestaurantPin,
                        style: ElevatedButton.styleFrom(
                          backgroundColor: AppTheme.primaryGreen,
                          foregroundColor: Colors.white,
                          elevation: 0,
                  ...
                  // MODE 12: RESTAURANT ONBOARDING STEP 6 - ADD MENU (MATCHING REFERENCE UI)
                    ...
                    // Upload Item Image Card (Dashed Border Card with Upload Icon & real ImagePicker)
                    GestureDetector(
                      onTap: () {
                        _showMediaPickerBottomSheet(
                          ...
                          onSelected: (name, base64Data) {
                            setState(() {
                              _uploadedMenuItemImage = name;
                              _uploadedMenuItemImageData = base64Data;
                            });
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                  content: Text('Dish image $name uploaded!'),
                                  backgroundColor: AppTheme.primaryGreen),
                            );
                          },
                        );
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(14),
                          border: Border.all(
                            color: (_uploadedMenuItemImageData != null ||
                                    _uploadedMenuItemImage != null)
                                ? AppTheme.primaryGreen
                                : Colors.grey[350]!,
                            width: 1.5,
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              _uploadedMenuItemImage != null
                                  ? 'Image Selected: $_uploadedMenuItemImage'
                                  : 'Upload Item Image (Camera or Gallery)',
                              style: GoogleFonts.poppins(
                                fontSize: 13,
                                fontWeight: FontWeight.w600,
                                color: (_uploadedMenuItemImageData != null ||
                        ...
                        child: DropdownButton<String>(
                          ...
                          items: [
                            ...
                            return DropdownMenuItem<String>(
                              ...
                              child: Text(cat,
                                  style: GoogleFonts.poppins(
                                      fontSize: 14, color: AppTheme.darkBlack)),
                            );
                          }).toList(),
                    ...
                    Row(
                      children: [
                        ...
                        InkWell(
                          onTap: () =>
                              setState(() => _selectedFoodType = 'Non-Veg'),
                          borderRadius: BorderRadius.circular(10),
                          child: Container(
                            padding: const EdgeInsets.symmetric(
                                horizontal: 16, vertical: 8),
                            decoration: BoxDecoration(
                              color: _selectedFoodType == 'Non-Veg'
                                  ? AppTheme.primaryGreen
                                      .withValues(alpha: 0.08)
                                  : Colors.white,
                              borderRadius: BorderRadius.circular(10),
                      ...
                      children: [
                        ...
                        IconButton(
                          icon: const Icon(Icons.add_circle_outline_rounded,
                              color: AppTheme.darkBlack, size: 24),
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      if ((item['flavours'] as List<String>?)
                                                  ?.isNotEmpty ==
                                              true ||
                                          (item['addOns'] as List<String>?)
                                                  ?.isNotEmpty ==
                                              true) ...[
                                        const SizedBox(height: 6),
                                        Wrap(
                    ...
                    // Next: Set PIN Primary CTA Button
                    SizedBox(
                      width: double.infinity,
                      height: 52,
                      child: ElevatedButton(
                        onPressed:
                            _isLoading ? null : _handleSaveRestaurantAddMenu,
                        style: ElevatedButton.styleFrom(
                          backgroundColor: AppTheme.primaryGreen,
                          foregroundColor: Colors.white,
                          elevation: 0,
                        ...
                        child: _isLoading
                            ? const CircularProgressIndicator(
                                color: Colors.white)
                            : Text(
                                _addedMenuItems.isEmpty
                                    ? 'Next: Set Login PIN'
                                    : 'Next: Set Login PIN (${_addedMenuItems.length} Items)',
                                style: GoogleFonts.poppins(
                                  fontSize: 16,
                                  fontWeight: FontWeight.bold,
                                ),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: _isLoading
                            ? null
                            : _handleSaveRestaurantOperationalDetails,
                        style: ElevatedButton.styleFrom(
                          backgroundColor: AppTheme.primaryGreen,
                          foregroundColor: Colors.white,
                          elevation: 0,
                  ...
                  // MODE 10: RESTAURANT ONBOARDING STEP 4 - BANK DETAILS (MATCHING REFERENCE UI)
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: _isLoading
                            ? null
                            : _handleSaveRestaurantBankDetails,
                        style: ElevatedButton.styleFrom(
                          backgroundColor: AppTheme.primaryGreen,
                          foregroundColor: Colors.white,
                          elevation: 0,
                  ...
                  // MODE 9: RESTAURANT ONBOARDING STEP 3 - DOCUMENTS (MATCHING REFERENCE UI)
                    ...
                    GestureDetector(
                      onTap: () {
                        _showMediaPickerBottomSheet(
                          title: 'Upload Food License Document',
                          onSelected: (name, base64Data) {
                            setState(() {
                              _uploadedFoodLicenseDoc = name;
                              _uploadedFoodLicenseDocData = base64Data;
                            });
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                  content: Text('Food license $name uploaded!'),
                                  backgroundColor: AppTheme.primaryGreen),
                            );
                          },
                        );
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(14),
                          border: Border.all(
                            color: _uploadedFoodLicenseDoc != null
                                ? AppTheme.primaryGreen
                                : Colors.grey[350]!,
                            width: 1.5,
                          ),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              _uploadedFoodLicenseDoc != null
                                  ? 'Food License Selected: $_uploadedFoodLicenseDoc'
                                  : 'Upload Food License Document (Camera or Gallery)',
                              style: GoogleFonts.poppins(
                                fontSize: 13,
                                fontWeight: FontWeight.w600,
                                color: _uploadedFoodLicenseDoc != null
                    ...
                    GestureDetector(
                      onTap: () {
                        _showMediaPickerBottomSheet(
                          title: 'Upload GST Certificate Document',
                          onSelected: (name, base64Data) {
                            setState(() {
                              _uploadedGstDoc = name;
                              _uploadedGstDocData = base64Data;
                            });
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                  content:
                                      Text('GST Certificate $name uploaded!'),
                                  backgroundColor: AppTheme.primaryGreen),
                            );
                          },
                        );
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              _uploadedGstDoc != null
                                  ? 'GST Certificate Selected: $_uploadedGstDoc'
                                  : 'Upload GST Certificate Document (Camera or Gallery)',
                              style: GoogleFonts.poppins(
                                fontSize: 13,
                                fontWeight: FontWeight.w600,
                                color: _uploadedGstDoc != null
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed:
                            _isLoading ? null : _handleSaveRestaurantDocuments,
                        style: ElevatedButton.styleFrom(
                          backgroundColor: AppTheme.primaryGreen,
                          foregroundColor: Colors.white,
                          elevation: 0,
                  ...
                  // MODE 8: RESTAURANT ONBOARDING STEP 2 - LOCATION (GOOGLE MAPS & LIVE GPS)
                  else if (_currentMode == AuthMode.restaurantLocation) ...[
                    // Auto GPS Detection & Search Banner
                    Container(
                      padding: const EdgeInsets.all(12),
                      decoration: BoxDecoration(
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  'Auto fetch live GPS coordinates or search any location',
                                  style: GoogleFonts.poppins(
                                      fontSize: 11, color: Colors.grey[600]),
                                ),
                              ],
                    ...
                    // Restaurant Address Field (Manual & Auto)
                    _buildFormLabel('Restaurant Full Address'),
                    const SizedBox(height: 6),
                    _buildCustomTextField(
                      ...
                      children: [
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              _buildFormLabel('Area / Colony'),
                              const SizedBox(height: 6),
                              _buildCustomTextField(
                                controller: _areaController,
                                hintText: 'e.g. Subhash Chowk',
                              ),
                            ],
                          ),
                        ),
                    ...
                    // Interactive Map Picker Card with Live Dynamic Pin
                    GestureDetector(
                      onTap: _showMapLocationPickerModal,
                      child: Container(
                        height: 200,
                        width: double.infinity,
                        decoration: BoxDecoration(
                        ...
                        child: ClipRRect(
                          ...
                          child: Stack(
                            children: [
                              ...
                              // Tap instruction banner (Top Right)
                              Positioned(
                                top: 10,
                                right: 10,
                                child: Container(
                              ...
                              Positioned(
                                ...
                                child: ElevatedButton.icon(
                                  onPressed: _showMapLocationPickerModal,
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed:
                            _isLoading ? null : _handleSaveRestaurantLocation,
                        style: ElevatedButton.styleFrom(
                          backgroundColor: AppTheme.primaryGreen,
                          foregroundColor: Colors.white,
                          elevation: 0,
                  ...
                  // MODE 7: RESTAURANT ONBOARDING STEP 1 - RESTAURANT DETAILS (MATCHING REFERENCE UI)
                    ...
                    Container(
                      ...
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton<String>(
                          ...
                          icon: const Icon(Icons.keyboard_arrow_down_rounded,
                              color: AppTheme.darkBlack),
                          items: ['Veg', 'Non-Veg', 'Both (Veg & Non-Veg)']
                              .map((String type) {
                            return DropdownMenuItem<String>(
                              value: type,
                              child: Text(type,
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed:
                            _isLoading ? null : _handleSaveRestaurantDetails,
                        style: ElevatedButton.styleFrom(
                          backgroundColor: AppTheme.primaryGreen,
                          foregroundColor: Colors.white,
                          elevation: 0,
                            ...
                            onChanged: (val) {
                              if (val.isNotEmpty && index < 5) {
                                FocusScope.of(context)
                                    .requestFocus(_regOtpFocusNodes[index + 1]);
                        ...
                        GestureDetector(
                          onTap: () {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                  content:
                                      Text('OTP Resent to your email/mobile!'),
                                  backgroundColor: AppTheme.primaryGreen),
                            );
                          },
                          child: Text(
                  ...
                  // MODE 1: PHONE NUMBER & PIN/OTP ENTRY (PRIMARY LOGIN SCREEN)
                    ...
                    Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 6),
                          Text(
                            'Sign in to manage your restaurant orders & menu',
                            textAlign: TextAlign.center,
                            style: GoogleFonts.poppins(
                              fontSize: 13,
                              fontWeight: FontWeight.w500,
                    ...
                    // Login Method Switcher Tabs (PIN vs OTP)
                    Container(
                      ...
                      child: Row(
                        children: [
                          Expanded(
                            child: GestureDetector(
                              onTap: () =>
                                  setState(() => _isPinLoginMode = true),
                              child: AnimatedContainer(
                                duration: const Duration(milliseconds: 200),
                                padding:
                                    const EdgeInsets.symmetric(vertical: 10),
                                decoration: BoxDecoration(
                                  color: _isPinLoginMode
                                      ? Colors.white
                                      : Colors.transparent,
                                  borderRadius: BorderRadius.circular(10),
                                  boxShadow: _isPinLoginMode
                                      ? [
                                          BoxShadow(
                                              color: Colors.black
                                                  .withOpacity(0.06),
                          ...
                          Expanded(
                            child: GestureDetector(
                              onTap: () =>
                                  setState(() => _isPinLoginMode = false),
                              child: AnimatedContainer(
                                duration: const Duration(milliseconds: 200),
                                padding:
                                    const EdgeInsets.symmetric(vertical: 10),
                    ...
                    _buildFormLabel('Registered Mobile Number'),
                    const SizedBox(height: 6),
                    Container(
                      decoration: BoxDecoration(
                        color: const Color(0xFFF9FAFB),
                    ...
                    if (_isPinLoginMode) ...[
                      const SizedBox(height: 16),
                      _buildFormLabel('4-Digit Login PIN'),
                      const SizedBox(height: 6),
                      Container(
                        decoration: BoxDecoration(
                          color: const Color(0xFFF9FAFB),
                        ...
                        child: TextField(
                          ...
                          keyboardType: TextInputType.number,
                          maxLength: 4,
                          obscureText: _isLoginPinObscured,
                          style: GoogleFonts.poppins(
                            fontSize: 18,
                            fontWeight: FontWeight.w700,
                            letterSpacing: 8,
                          ...
                          decoration: InputDecoration(
                            ...
                            suffixIcon: IconButton(
                              ...
                              onPressed: () => setState(() =>
                                  _isLoginPinObscured = !_isLoginPinObscured),
                            ),
                            contentPadding: const EdgeInsets.symmetric(
                                horizontal: 14, vertical: 14),
                            border: InputBorder.none,
                    ...
                    // Primary Action Button (Sign In with PIN or Get Started / Send OTP)
                    SizedBox(
                      width: double.infinity,
                      height: 52,
                      child: ElevatedButton(
                    ...
                    // Switch Mode Helper Button
                    Center(
                      child: TextButton(
                        onPressed: () {
                          setState(() {
                            _isPinLoginMode = !_isPinLoginMode;
                          });
                        },
                        child: Text(
                          _isPinLoginMode
                              ? 'Login using SMS OTP instead'
                              : 'Already have a 4-Digit PIN? Sign In with PIN',
                          style: GoogleFonts.poppins(
                            fontSize: 13,
                            fontWeight: FontWeight.w600,
                            color: AppTheme.primaryGreen,
    ...
    return Column(
      ...
      children: [
        ...
        _buildPartnerFeatureRow(
          ...
          subtitle:
              'Dedicated rider network ensuring speedy 25-30 min door-to-door delivery.',
        ),
        const SizedBox(height: 10),
        _buildPartnerFeatureRow(
          ...
          subtitle:
              'Transparent payouts transferred straight to your bank with zero hidden cuts.',
        ),
        const SizedBox(height: 10),
        _buildPartnerFeatureRow(
          ...
          subtitle:
              'Real-time loud audio alerts, instant item toggle & live price management.',
        ),
        const SizedBox(height: 16),
...
class _MapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(
        Offset(size.width * 0.2, size.height * 0.3), 60, greenAreaPaint);
    canvas.drawCircle(
        Offset(size.width * 0.8, size.height * 0.7), 80, greenAreaPaint);
    ...
    roadPath.cubicTo(size.width * 0.3, size.height * 0.2, size.width * 0.6,
    ...
    riverPath.cubicTo(size.width * 0.7, size.height * 0.3, size.width * 0.4,
        size.height * 0.7, size.width * 0.8, size.height);
