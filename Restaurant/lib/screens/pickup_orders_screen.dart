...
class _PickupOrdersScreenState extends State<PickupOrdersScreen> {
  ...
  void initState() {
    ...
    _pollingTimer = Timer.periodic(const Duration(seconds: 8), (_) {
      if (mounted) _fetchLivePickupOrders(isSilent: true);
    });
  }
  ...
  Future<void> _fetchLivePickupOrders({bool isSilent = false}) async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        for (var o in rawOrders) {
          ...
          if (ord.isSelfPickup || ord.orderType.toLowerCase() == 'pickup') {
            parsedList.add(ord);
  ...
  int _getCountForStatus(String status) {
    ...
    if (status == 'Placed') {
      return _pickupOrders.where((o) => o.status == 'Placed' || o.status == 'Pending').length;
    }
    if (status == 'Ready') {
      return _pickupOrders.where((o) => o.status == 'Ready' || o.status == 'Ready for Pickup').length;
    }
    if (status == 'Picked Up') {
      return _pickupOrders.where((o) => o.status == 'Picked Up' || o.status == 'Delivered' || o.status == 'Handed Over').length;
    }
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Text(
                  '₹${_pickupRevenue.toStringAsFixed(2)}',
                  style: GoogleFonts.poppins(
                    fontSize: 22,
                    fontWeight: FontWeight.bold,
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 8),
                        if (order.pickupOtp != null && order.pickupOtp!.isNotEmpty)
                          Container(
                            ...
                            child: Text('OTP: ${order.pickupOtp}', style: GoogleFonts.poppins(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.black87)),
                          ),
                      ],
                    ),
