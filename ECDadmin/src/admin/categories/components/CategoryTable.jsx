...
const CategoryTable = () => {
  ...
  const initialTab = (tabParam === "1" || tabParam === "subcategories" || tabParam === "subcategory") ? 1 : 0;
  ...
  const [activeTab, setActiveTab] = useState(initialTab); // 0 = Main Categories, 1 = Subcategories

  useEffect(() => {
    if (tabParam === "1" || tabParam === "subcategories" || tabParam === "subcategory") {
      setActiveTab(1);
    } else if (tabParam === "0" || tabParam === "main") {
      setActiveTab(0);
    }
  }, [tabParam]);
  ...
  const handleSaveCategory = async () => {
    try {
      ...
      // Preserve existing image if editing and image was not explicitly deleted
      if (editingCategory && !payload.image && editingCategory.image) {
        payload.image = editingCategory.image;
      }
      ...
      <Dialog open={modalOpen} onClose={() => setModalOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent dividers>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2, pt: 1 }}>
            ...
            <TextField
              label="Starting Price (₹) [App me 'FROM ₹X' dikhega]"
              type="number"
              fullWidth
            ...
            <Box sx={{ border: "1px dashed #CBD5E1", p: 2, borderRadius: 2, bgcolor: "#F8FAFC" }}>
              <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: 600 }}>Category Image (ImageKit CDN Upload)</Typography>
              <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                {formData.image ? (
                  <Box sx={{ position: "relative" }}>
                    <Box component="img" src={formData.image} alt="Preview" sx={{ width: 64, height: 64, borderRadius: 2, objectFit: "cover" }} />
                    <IconButton
                      size="small"
                      onClick={() => setFormData({ ...formData, image: "" })}
                      sx={{ position: "absolute", top: -8, right: -8, bgcolor: "red", color: "white", p: 0.3, "&:hover": { bgcolor: "darkred" } }}
                    >
                ...
                <Button
                  ...
                  variant="contained"
                  size="small"
                  sx={{ backgroundColor: PRIMARY_COLOR, textTransform: "none", fontWeight: 600 }}
                >
